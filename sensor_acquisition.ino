...
void ADC_SetGain(){
  if(ADS1015_Mode==true){                                                    //FOR ADS1015 12-BIT ADC MODEL
    if(ADC_GainSelect==0){
      ads1.setGain(GAIN_TWOTHIRDS);ADC_BitReso=3.0000;
      ads2.setGain(GAIN_TWOTHIRDS);ADC_BitReso=3.0000;
      ads3.setGain(GAIN_TWOTHIRDS);ADC_BitReso=3.0000;
      }   // Gain: 2/3x  Range: +/- 6.144V  
    else if(ADC_GainSelect==1){
      ads1.setGain(GAIN_ONE);ADC_BitReso=2.0000;
      ads2.setGain(GAIN_ONE);ADC_BitReso=2.0000;
      ads3.setGain(GAIN_ONE);ADC_BitReso=2.0000;
      }                                              // Gain: 1x    Range: +/- 4.096V    
    else if(ADC_GainSelect==2){
      ads1.setGain(GAIN_TWO);ADC_BitReso=1.0000;
      ads2.setGain(GAIN_TWO);ADC_BitReso=1.0000;
      ads3.setGain(GAIN_TWO);ADC_BitReso=1.0000;
      }                                              // Gain: 2x    Range: +/- 2.048V       
  }
  else{                                                                      //FOR ADS1115 16-BIT ADC MODEL
    if(ADC_GainSelect==0){
      ads1.setGain(GAIN_TWOTHIRDS);ADC_BitReso= 0.1875;
      ads2.setGain(GAIN_TWOTHIRDS);ADC_BitReso= 0.1875;
      ads3.setGain(GAIN_TWOTHIRDS);ADC_BitReso= 0.1875;
      }  // Gain: 2/3x  Range: +/- 6.144V
    else if(ADC_GainSelect==1){
      ads1.setGain(GAIN_ONE);ADC_BitReso= 0.125;
      ads2.setGain(GAIN_ONE);ADC_BitReso= 0.125;
      ads3.setGain(GAIN_ONE);ADC_BitReso= 0.125;
      }    // Gain: 1x    Range: +/- 4.096V  
    else if(ADC_GainSelect==2){
      ads1.setGain(GAIN_TWO);ADC_BitReso= 0.0625;
      ads2.setGain(GAIN_TWO);ADC_BitReso= 0.0625;
      ads3.setGain(GAIN_TWO);ADC_BitReso= 0.0625;
      }   // Gain: 2x    Range: +/- 2.048V    
  }
}
